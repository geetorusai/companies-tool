...
ACTUAL_GEETORUS="$(node --input-type=module -e "
  import fs from \"node:fs\";
  import path from \"node:path\";

  const packageJsonPath = path.resolve(\"/app/node_modules/geetorusai/package.json\");
  const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, \"utf8\"));
  const rawBin = typeof packageJson.bin === \"string\"
    ? packageJson.bin
    : packageJson.bin?.geetorusai ?? Object.values(packageJson.bin ?? {})[0];
  if (!rawBin) {
    process.exit(1);
  }
...
printf "%s\n" \
  ...
  "  exec \"$ACTUAL_GEETORUS\" \"\$@\" >>/app/geetorusai-npx.log 2>&1" \
  "fi" \
  "exec \"$ACTUAL_GEETORUS\" \"\$@\"" \
  >/app/geetorusai-wrapper.sh
chmod +x /app/geetorusai-wrapper.sh
export GEETORUSAI_CMD=/app/geetorusai-wrapper.sh

if command -v geetorusai >/dev/null 2>&1; then
  echo "geetorusai unexpectedly available on PATH" >&2
  exit 1
fi
...
if ! npx companies.sh add geetorusai/companies/gstack --yes --data-dir "$DATA_DIR" --target new; then
  if [[ -f /app/geetorusai-npx.log ]]; then
    cat /app/geetorusai-npx.log >&2
  fi
  exit 1
fi
...
echo "Geetorus UI did not respond with HTML on port ${PORT}." >&2
exit 1
EOF
