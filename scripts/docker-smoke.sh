...
docker run --rm \
  ...
  bash -lc '
    ...
    cat >/tmp/run-companies-smoke.sh <<'\''EOF'\''
#!/usr/bin/env bash
set -euo pipefail
...
ACTUAL_GEETORUS="$(node --input-type=module -e "
  ...
  const packageJsonPath = require.resolve(\"geetorusai/package.json\");
...
printf "%s\n" \
  ...
  "  exec \"$ACTUAL_GEETORUS\" \"\$@\" >>/app/geetorusai.log 2>&1" \
  "fi" \
...
if ! companies.sh add ./fixtures/minimal-company --yes --data-dir "$DATA_DIR" --target new; then
  if [[ -f /app/geetorusai.log ]]; then
    cat /app/geetorusai.log >&2
  fi
  exit 1
fi
...
exit 1
EOF
    chmod +x /tmp/run-companies-smoke.sh
    su node -s /bin/bash -c /tmp/run-companies-smoke.sh
  '
