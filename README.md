...
# GitHub tree URL
npx companies.sh add https://github.com/geetorusai/companies/tree/main/gstack
...
| `-p, --provider <provider>` | Destination orchestrator. Default: `geetorus`.                      |
...
npx companies.sh add geetorusai/companies/gstack --target new

# Import into an existing company, pick from list
npx companies.sh add geetorusai/companies/gstack --target existing

# Import into a specific company without prompts
npx companies.sh add geetorusai/companies/gstack --target existing -C <company-id>

# Preview what would be imported
npx companies.sh add ./my-company --target existing -C <company-id> --dry-run

# Import specific agents only
npx companies.sh add geetorusai/companies/gstack --agents ceo,cto

# Import everything (agents, projects, tasks, skills)
npx companies.sh add geetorusai/companies/gstack --include company,agents,projects,tasks,skills

# Non-interactive usage for scripts or CI
npx companies.sh add geetorusai/companies/gstack --target new -y

# Use a specific Geetorus instance
npx companies.sh add geetorusai/companies/gstack --connection custom-url --api-base http://127.0.0.1:3100
```
...
2. Connect to the target orchestrator ([Geetorus](https://geetorus.ing) by default).
...
| Mode         | Behavior |
| ------------ | -------- |
| `auto`       | Checks local Geetorus config, falls back to `http://127.0.0.1:3100`, runs `geetorusai onboard --yes` if no config exists, starts `geetorusai run` if the server is not up. |
| `custom-url` | Skips local bootstrap; expects a reachable instance at `--api-base`. |

### Geetorus Connection Flags

| Flag                    | Description                           |
| ----------------------- | ------------------------------------- |
| `-c, --config <path>`   | Path to a Geetorus config file.      |
| `-d, --data-dir <path>` | Geetorus data directory root.        |
| `--context <path>`      | Path to a Geetorus CLI context file. |
| `--profile <name>`      | Geetorus CLI context profile name.   |
| `--api-base <url>`      | Geetorus API base URL override.      |
| `--api-key <token>`     | Geetorus API key override.           |

`companies.sh` requires `geetorusai >= 2026.325.0`. Fresh installs resolve a bundled release automatically.

## Adding a Provider

`companies.sh` is provider-based. [Geetorus](https://geetorus.ing) is the first supported provider, but the architecture accepts additional agent orchestrators.
...
| -------------------------------------- | ------------------------------------------------------------------ |
| `GEETORUSAI_CMD`                      | Override the `geetorusai` executable or full command.              |
| `COMPANIES_GEETORUS_START_TIMEOUT_MS` | Local Geetorus readiness timeout in milliseconds. Default: `120000`. |
...
### Use a different `geetorusai` build

```bash
export GEETORUSAI_CMD=/path/to/geetorusai

# With prefix arguments
export GEETORUSAI_CMD="pnpm --dir /path/to/geetorus geetorusai"
```
...
```bash
export COMPANIES_GEETORUS_START_TIMEOUT_MS=180000
```
