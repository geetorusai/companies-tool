...
test("prepareInstallTelemetry resolves GitHub company slugs via the GitHub contents API", async () => {
  ...
  await withEnv({
    ...
    await withMockFetch(async (input) => {
      ...
      assert.match(url, /api\.github\.com\/repos\/geetorusai\/company-template\/contents\/company\/COMPANY\.md\?ref=main$/);
      ...
      const telemetry = await prepareInstallTelemetry(
        "https://github.com/geetorusai/company-template/tree/main/company",
        "existing",
      );
...
test("prepareInstallTelemetry resolves GitHub company slugs from owner/repo/path shorthand", async () => {
  ...
  await withEnv({
    ...
    await withMockFetch(async (input) => {
      const url = String(input);
      assert.match(url, /api\.github\.com\/repos\/geetorusai\/companies\/contents\/gstack\/COMPANY\.md$/);
...
test("sendInstallCompletedTelemetry posts the expected envelope and never throws", async () => {
  ...
  await withMockFetch(async (_input, init) => {
    ...
    await sendInstallCompletedTelemetry({
      ...
      ingestUrl: "https://telemetry.geetorus.ing/ingest",
