...
`companies.sh` depends on `geetorusai@^2026.325.0`, so fresh installs resolve a compatible stable Geetorus release no older than `2026.325.0`.

That means:

- canary publishes should still be validated against the current Geetorus canary when a change depends on unreleased Geetorus behavior
- stable `companies.sh` releases keep a minimum compatible Geetorus floor in `package.json`
- if a future `companies.sh` change needs a newer Geetorus capability, bump that floor and the runtime version gate together
...
If the package behavior changed in a risky way, also run a real dry-run import against a Geetorus instance.
