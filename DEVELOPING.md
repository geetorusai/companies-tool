...
## Hand-Test Against A Local Geetorus Instance

Use this when you already have Geetorus on your machine and want to watch the imported company appear in the local UI.

```bash
pnpm install
pnpm build

# If Geetorus is not already running, start it in another terminal.
./node_modules/.bin/geetorusai run

# Import the fixture through the current local Geetorus instance.
...
That path uses the bundled stable `geetorusai` release, runs `geetorusai onboard --yes` when needed, and starts the local server automatically before importing.

## Hand-Test Against A Docker Geetorus Instance

Use this when you want the automated clean-room Docker smoke test. It packages the current CLI, starts a fresh Linux container with Node 20, installs only production dependencies inside that container, then verifies the wrapper bootstraps Geetorus, imports the fixture, and serves the UI on the container loopback interface without any standalone `geetorusai` binary on `PATH`.
...
That smoke test starts from a clean Node 20 container, installs only the Debian prerequisites, switches to the non-root `node` user, and runs the current local tarball through `npx`. It verifies the full success condition end to end: no standalone `geetorusai` on `PATH`, `npx companies.sh add ...` bootstraps local Geetorus, imports `geetorusai/companies/gstack`, lists the imported company, and serves the Geetorus UI over container-local HTTP.
...
docker run --rm -it \
  -e COMPANIES_GEETORUS_START_TIMEOUT_MS=180000 \
  -e HOST=127.0.0.1 \
  -e PORT=3210 \
  -e SERVE_UI=true \
  -e GEETORUS_OPEN_ON_LISTEN=false \
...
That loopback binding is intentional: Geetorus quickstart uses `local_trusted`, which requires `127.0.0.1` inside the container. The automated smoke test verifies the UI over container-local HTTP rather than Docker port publishing.

### Viewing the Geetorus UI from your host browser

The default manual shell setup binds to `127.0.0.1` inside the container, which means the host cannot reach it. To open the Geetorus UI in your host browser you need two changes:

1. **Bind to all interfaces** — set `HOST=0.0.0.0` so Geetorus listens on the container's external interface, not just loopback.
...
npx companies.sh@canary add geetorusai/companies/gstack
npx companies.sh@canary list
```

Run that `npx` command as `node`, not `root`. Local Geetorus bootstrap uses embedded services that can stall under Linux root sessions, so `companies.sh` now fails fast there and tells you to switch users instead of waiting forever.
...
- wait for the local Geetorus bootstrap note to finish on first run
- confirm the imported company appears in `npx companies.sh@canary list`

The important behavior is that the CLI should keep advancing after `auto` instead of appearing to stall there. On a fresh container the first bootstrap can still take a while, but the prompt flow now reaches the target selection before local Geetorus startup begins.

## Dry-Run Smoke Test

Use this to verify the wrapper can reach a local Geetorus instance without applying writes:

```bash
export GEETORUSAI_CMD="pnpm --dir /path/to/geetorus run geetorusai"
...
npx companies.sh add geetorusai/companies/gstack \
  ...
  --api-key "$GEETORUS_API_KEY"
```

Expected result: the command reaches the Geetorus import preview path and does not create a company because `--dry-run` routes to preview only. Use a board-scoped API key or board-authenticated context for this check because preview endpoints may reject agent-scoped credentials.
